...
import type { StoryPublicationPersistence } from "@/application/story-publications";
import { STORY_STATES, type Story } from "@/domain/editorial";

/** Only an approved Story can be published; the domain enforces the same rule. */
const PUBLISHABLE_STATES = new Set(["approved"]);

export class PostgresStoryPublicationInvariantError extends Error {
  constructor() {
    super("PostgreSQL Story publication persistence returned an invalid or impossible result.");
    this.name = "PostgresStoryPublicationInvariantError";
  }
}
...
function decodeStory(row: QueryResultRow): Story {
  if (
    ...
    throw new PostgresStoryPublicationInvariantError();

  return structuredClone(row.payload) as unknown as Story;
}

export function createPostgresStoryPublicationPersistence(options: {
  readonly pool: Pool;
  ...
  return {
    async persist(command) {
      const conflict = () => ({
        ok: false as const,
        error: {
          code: "STORY_PUBLICATION_CONFLICT" as const,
          message: "The Story changed before publication was recorded.",
          storyId: command.expectedStory.id,
        },
      });
      if (
        !PUBLISHABLE_STATES.has(command.expectedStory.state) ||
        command.story.state !== "published" ||
        ...
        command.transitionReceipt.nextState !== "published" ||
