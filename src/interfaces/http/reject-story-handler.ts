import type { RejectStoryResult } from "@/application/story-rejections";
...
function status(result: RejectStoryResult): number {
  if (result.ok) return 201;
  if (result.error.code === "STORY_NOT_FOUND") return 404;
  if (["INVALID_TRANSITION", "STORY_REJECTION_CONFLICT"].includes(result.error.code)) return 409;
  return 422;
}

export function createRejectStoryHttpHandler(dependencies: {
  ...
  return async (request: Request, context: StoryRouteContext): Promise<Response> => {
    ...
    try {
      ...
      const rejectedBy: OperatorActor = {
        type: "operator",
        operatorId: operatorId(configured.trim()),
      };
      const parameters = await context.params;
      const result = await dependencies.getRuntime().rejectStory({
      ...
      return respond(error("INTERNAL_SERVER_ERROR", "The Story could not be rejected."), 500);
    }
  };
}
