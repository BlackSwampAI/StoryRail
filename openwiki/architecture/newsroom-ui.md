...
| `StoryWorkspace`          | `story-workspace.tsx`           | `GET /api/stories/[storyId]`, `POST .../assignment-proposals`, `POST .../assignments`, `POST .../writer-drafts`, `POST .../writer-revisions`, `POST .../review-submissions`, `POST .../director-reviews`, `POST .../review-decisions` via `story-client.ts` |
...
`story-client.ts` creates Stories, lists them, inspects a selected Story, attaches Sources, assigns Stories, generates Assignment Editor proposals, creates Writer drafts, creates Writer revisions, submits reviews, runs Director reviews, and records review decisions, with strict shape validation of every returned domain object (including the `article_revision` Writer run variant and revisions 1–3). `newsroom-shell.tsx` uses `actorLabel` to render operator/agent provenance and `safeUrl` to validate displayed URLs before rendering them as links.
