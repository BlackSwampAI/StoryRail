...
`postgres-source-extraction-decoder.ts` decodes a persisted extraction row back into the `SuccessfulSourceExtraction` / `FailedSourceExtraction` union and is shared by the inspection and inbox adapters.

## Runtime composition (`src/runtime`)

`src/runtime/index.ts` exports the Source-evidence, evidence-preparation, and Story runtime factories and their configuration loaders.
...
- `createStoryRuntime(options)` builds one `pg.Pool` and constructs the story, attachment, inspection, listing, source-inbox, and source-triage repositories. It wires `createStory`, `attachSourceToStory`, `inspectStory`, `listStories`, `listPendingSources`, and `recordSourceTriageDecision`.
- Returns a frozen `StoryRuntime` exposing those six operations plus an idempotent `close()`.
- `createStoryRuntimeFromEnvironment(options)` reads `STORYRAIL_DATABASE_URL` from `process.env` (or an injected environment).

## Injectable seams

Both runtimes accept optional `createPool`, `fetch`, `createUuid`, and `now` overrides. The runtime unit tests (`source-evidence-runtime.test.ts`, `story-runtime.test.ts`) inject Pool, fetch, UUID, and clock substitutes, so they require no real PostgreSQL or Firecrawl access. The PostgreSQL integration tests (`src/adapters/source-persistence/postgres-source-repositories.test.ts`) run the contracts against real PostgreSQL 18.4.

## Server providers (`src/server`)

`src/server/source-evidence-runtime-provider.ts` and `src/server/story-runtime-provider.ts` are lazy singletons: `get()` builds the runtime from environment on first call and caches it. Next.js route handlers receive `getRuntime: provider.get`, so the pool is created only when a request first needs it.
