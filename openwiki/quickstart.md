...
- [Newsroom UI shell](architecture/newsroom-ui.md) — resizable desk, staff sidebar, Story workspace with assignment, writing, Writer revision, review submission, Director review, and operator decision, and SafeMarkdown.
...
A rejection UI and publishing/export remain planned. The domain state machine already permits `in_review` → `rejected` and `approved` → `published`, but no application workflow or UI exercises those transitions yet.
