...
No application route, handler, or server action invokes the runtime, so operators cannot yet invoke the combined workflow through the newsroom. The newsroom still uses fixtures. No Pool singleton or framework lifecycle cache is installed, and migrations must be applied externally. Story creation and Source attachment remain deferred. StoryRail is therefore not yet an end-to-end operational editorial workflow, deployed system, or production-ready product.
