...
The first five steps are implemented today, including supervised Assignment Editor proposal generation and operator-approved durable Assignment. The remaining writing, review, Article, and publishing steps describe the planned completion of this vertical slice.
...
The provider-neutral structured-model boundary is backed by LangChain's dedicated `ChatOpenRouter` adapter and strict structured output, with StoryRail performing final Zod validation. OpenRouter is the first implemented provider. Evidence preparation and supervised Assignment Editor execution use separate lazy server-only runtimes and separate explicit model variables. `STORYRAIL_ASSIGNMENT_EDITOR_MODEL` is never inferred from a profile or reused from `STORYRAIL_EVIDENCE_PREPARATION_MODEL`. Ordinary Story, Inbox, triage, inspection, Agent Profile, and manual Assignment paths do not require OpenRouter configuration.
