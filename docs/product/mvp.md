...
The newsroom's eight Story queues are backed by persisted Stories. Story inspection includes Sources, Assignment, Writer Profile, append-ordered AgentRuns and transitions, the Article with immutable revisions, and append-ordered ReviewDecisions. A successful Director run leaves the Story In Review; only the operator decision moves it to Approved or Changes Requested. The operator may separately reject a Story from Intake, Assigned, In Progress, In Review, or Changes Requested; StoryRail requires an editorial reason and atomically preserves the terminal transition receipt without deleting prior work. After reload, the newsroom reads that explanation from the authoritative rejection transition rather than browser state or a denormalized Story field.
